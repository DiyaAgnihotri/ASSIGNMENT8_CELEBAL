...
def set_background():
    st.markdown("""
        <style>
        .stApp {
            background-image: url('https://plus.unsplash.com/premium_photo-1681487769650-a0c3fbaed85a?fm=jpg&q=60&w=3000&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8ZmluYW5jZXxlbnwwfHwwfHx8MA%3D%3D');
...
def display_system_info():
    ...
    with col1:
        ...
        for description, info in file_info.items():
            if info['exists']:
                ...
                st.text(f"❌ {description}: Not found")

    with col2:
        st.markdown("**🚀 Optimization Features:**")
        features = [
            "✅ Compressed RAG cache (.gz format)",
            "✅ Smart data sampling (500 samples)",
            "✅ Float16 embeddings for size reduction",
            "✅ Gzip compression (level 9)",
            "✅ GitHub LFS compatible",
            "✅ Auto-fallback to regular cache"
        ]
    ...
    if old_cache_exists and not compressed_exists:
        ...
        if cache_size > 25:
            st.markdown("""
            <div class="warning-box">
                ⚠️ <strong>GitHub Size Warning:</strong> Your cache file is larger than 25MB.
...
def main():
    ...
    with st.container():
        ...
        with col1:
            st.markdown('<div class="status-badge status-ready">🤖 AI Model Ready</div>', unsafe_allow_html=True)
        with col2:
            st.markdown('<div class="status-badge status-ready">💬 Chatbot Online</div>', unsafe_allow_html=True)
        with col3:
            compressed_cache_exists = os.path.exists('rag_cache_compressed.gz')
            if compressed_cache_exists:
                st.markdown('<div class="status-badge status-compressed">📦 Compressed Cache</div>', unsafe_allow_html=True)
            else:
                st.markdown('<div class="status-badge status-ready">⚡ Fast Predictions</div>', unsafe_allow_html=True)

        # Main tabs
        tab1, tab2, tab3, tab4 = st.tabs(["🎯 Loan Prediction", "💬 AI Assistant", "📊 Model Info", "⚙️ System Info"])
        ...
        with tab4:
                ...
                if st.button("📦 Create Compressed Cache", key="compress_btn"):
                    with st.spinner("Creating compressed RAG cache..."):
                        try:
                            ...
                            st.success("✅ Compressed cache created successfully!")
                            st.rerun()
                        except Exception as e:
                            st.error(f"❌ Error creating cache: {str(e)}")
            ...
            with col2:
                if st.button("🧹 Clean Old Files", key="clean_btn"):
                    ...
                    if cleaned:
                        st.success(f"✅ Cleaned files: {', '.join(cleaned)}")
                        st.rerun()
                    else:
                        st.info("ℹ️ No old files to clean")

            # GitHub deployment tips
            st.markdown("### 🚀 GitHub Deployment Tips")
            st.markdown("""
        ...
        st.markdown("""
        <div class="footer">
            <strong>🚀 Built with Advanced AI & Machine Learning</strong><br>
            Made with ❤️ by <strong>Diya Agnihotri</strong> | Powered by Streamlit & XGBoost<br>
